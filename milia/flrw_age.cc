...
namespace
{
    double helper_fun_time2(double z,void* pars)
    {
        milia::metrics::flrw* a=static_cast<milia::metrics::flrw*>(pars);
        return 1./((1+z)*(a->helper(z)));
...
namespace milia
{
    namespace metrics
    {
        ...
        double flrw::tolz(double z) const
        {
            ...
            case OV_2:
                //ol=0 om>1
                return m_t_h*(prez/(1.0+z)-m_om/sqrt(-pre0)*atan(sqrt(-pre0)/prez))/pre0;
            case OV_3:
                // ol=0 om=1
        ...
        {
            return m_t_h*asinh(1.0/((1.0+z)*sqrt(1.0/m_ov-1.0)))/sqrt(m_ov);
        }
        ...
        double flrw::ta1(double z) const
        {
            ...
            if(fabs(1.0+n*sin_phi*sin_phi)<EPS) {
                ...
                if(fabs(1.0+n*sin_phi*sin_phi)<EPS) {
                    ...
                    const double arg2=(1.0-arg1)*sqrt(arg1*arg1+(arg1+M_SQRT1_2)*(1+M_SQRT1_2));
                    const double arg3=(M_SQRT2-1.0)*(arg1+M_SQRT2+1.0)*sqrt((1.0+M_SQRT1_2)*(M_SQRT1_2-arg1));
                    return 0.25*m_t_h/sqrt(m_ov)*((M_SQRT2-1.0)*gsl_sf_ellint_F(phi,0.5*sqrt(1+2*M_SQRT2),PREC)+log(fabs((arg2+arg3)/(arg2-arg3))));
                }
                //        EQUATION 8.
                else {
                    // En mi paper, 7
            ...
            //       EQUATION 10.
                ...
                arg3=-0.5*(sqrt(A/(m_kap*(y1+1.0)))/(m_kap*y1))*log(fabs((1.0-hm)/(1.0+hm)));
        ...
        double flrw::ta2(double z) const
        {
            ...
            if(m_case==A2_1) {
                ...
                return -m_t_h*(M_SQRT3*log(1.0-2.0/(sqrt(1.0/3.0-arg)+1.0))+log(1.0+2.0/(sqrt(1.0-3.0*arg)-1.0)))/sqrt(m_ov);
            }
            //       EQUATION 15.
            if(m_case==A2_2) {
                int status=0;
        ...
        {
            // om+ol=1
            return 2.0*m_t_h*asinh(sqrt(m_ov/(m_om*(1.0+z*(3.0+z*(3.0+z))))))/(3.0*sqrt(m_ov));
        }
        ...
        double flrw::integrate_time(double z) const
        {
            ...
            double result,error;
            gsl_function F;
            F.function=&helper_fun_time2;
            F.params=static_cast<void*>(const_cast<flrw*>(this));
            const int status=gsl_integration_qagiu(&F,z,0,1e-7,1000,w,&result,&error);
            if(status) {
                std::cerr<<"error: "<<gsl_strerror(status)<<" on "
