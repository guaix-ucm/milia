  ...
  namespace metrics
  {
    ...
    double flrw::age() const
    {
      switch (m_case)
      {
        case OM_OV_0:
          ...
          break;
        case OV_1:
          return m_t_h * (1 - m_om / sqrt(1 - m_om) * atanh(sqrt(1 - m_om)))
              / (1 - m_om);
        case OV_2:
          return m_t_h * (1 - m_om / sqrt(m_om - 1) * atan(sqrt(m_om - 1)))
              / (1 - m_om);
        case OV_EDS:
          return 2 * m_t_h / 3;
          break;
    ...
    double flrw::tolz(double z) const
    {
      ...
      switch (m_case)
      {
        ...
        case OV_2:
          //ol = 0 om > 1
          return m_t_h * (prez / (1 + z) - m_om / sqrt(-pre0) * atan(
              sqrt(-pre0) / prez)) / pre0;
        case OV_EDS:
          // ol = 0 om = 1
    ...
    double flrw::ta1(double z) const
    {
      ...
      if (abs(crit10) < FLRW_EQ_TOL)
      {
        ...
        if (abs(crit8) < FLRW_EQ_TOL)
        {
          ...
          return 0.25 * m_t_h / sqrt(m_ov) * ((M_SQRT2 - 1) * ellint_1(0.5
              * sqrt(1 + 2 * M_SQRT2), phi) + log(abs((arg2 + arg3) / (arg2
              - arg3))));
        }
        //        EQUATION 8.
      ...
      else
      {
        ...
        arg3 = -0.5 * (sqrt(A / (m_kap * (y1 + 1))) / (m_kap * y1)) * log(abs(
            (1.0 - hm) / (1.0 + hm)));
        return m_t_h * m_om / (sqrt(A * abs(pow<3> (m_ok)))) * (arg1 + arg2
            + arg3);
      }
      return -1.0;
    }
    ...
    {
      // om + ov = 1
      return (2. * m_t_h / (3. * sqrt(1 - m_om))) * asinh(sqrt((1. / m_om - 1)
          / (pow<3> (1 + z))));
    }
