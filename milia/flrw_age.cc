...
namespace milia
{
    namespace metrics
    {
        ...
        // m_ov = 0 CASE: OL_1,OL_2,OL_3
        double flrw::tovz(double z) const
        {
        ...
        double flrw::ta1(double z) const
        {
            ...
            if (fabs(1.0 + n * gsl_pow_2(sin_phi)) < EPS) {
                ...
                if (fabs(1.0 + n * gsl_pow_2(sin_phi)) < EPS) {
                    ...
                    return 0.25*m_t_h/sqrt(m_ov)*((M_SQRT2-1.0)*gsl_sf_ellint_F(phi,
        ...
        double flrw::ta2(double z) const
        {
            ...
            if (m_case==A2_1) {
                ...
                return -m_t_h*(M_SQRT3*log(1.0-2.0/(sqrt(1.0/3.0-arg)+1.0))+log(1.0+2.0
                               /(sqrt(1.0-3.0*arg)-1.0)))/sqrt(m_ov);
            }
            //       EQUATION 15.
            if (m_case==A2_2) {
                int status=0;
        ...
        {
            // m_om+m_ol=1
            return 2.0*m_t_h*asinh(sqrt(m_ov/(m_om*(1.0+z*(3.0+z*(3.0+z))))))/(3.0
                    *sqrt(m_ov));
        }
