...
double flrw::dl(double z) const {
		...
		//om+ol!=1 b<0 || b>2
	{
		...
		if (m_b > 0)
			return m_r_h * (1 + z) / sqrt(abs(m_ok)) * sin(g * (ellint_1(k,
					phi0) - ellint_1(k, phi)));
		else
			return m_r_h * (1 + z) / sqrt(abs(m_ok)) * sinh(g * (ellint_1(k,
					phi0) - ellint_1(k, phi)));
	}
	case A2_1:
		// b=2
		...
		// 0<b<2
	{
		...
		return m_r_h * (1. + z) / sqrt(abs(m_ok)) * sin(g * (ellint_1(k, phi0)
				- ellint_1(k, phi)));
	}
	case OM_OV_1: {
		// om+ol=1
		...
		return m_r_h * (1 + z) / (c1 * sqrt(m_om) * sqrt(arg0)) * (ellint_1(k,
				phi0) - ellint_1(k, phi));
	}
		break;
	}
