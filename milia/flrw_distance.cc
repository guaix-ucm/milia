  ...
  namespace metrics
  {
    ...
    double flrw::dl(double z) const
    {
      ...
      case A1:
        //om+ol!=1 b<0 || b>2
      {
        const double v = pow(m_kap * (m_b - 1.) + sqrt(m_b * (m_b - 2.)), 1.
            / 3.);
        ...
        if (m_b > 0)
          return m_r_h * (1 + z) / sqrt(fabs(m_ok)) * sin(g * (gsl_sf_ellint_F(
              phi0, k, PREC) - gsl_sf_ellint_F(phi, k, PREC)));
        else
          return m_r_h * (1 + z) / sqrt(fabs(m_ok)) * sinh(
              g * (gsl_sf_ellint_F(phi0, k, PREC) - gsl_sf_ellint_F(phi, k,
                  PREC)));
      }
      case A2_1:
        // b=2
      ...
      case OM_OV_1: {
        ...
        return m_r_h * (1 + z) / (c1 * sqrt(m_om) * sqrt(arg0))
